...
3,aspect_ratio = 1, gap = (4,4)):
	...
	if parent_view is None:
		...
		import math
		i = views_per_row
		view_width = math.floor(parent_view.width/i) - (i + 1) * x_gap
...
def horiz(parent_view = None, ref_view = None, auto_sizing = True, view_list = list(), dim = (50,50), gap = (4,4)):
	import ui
	view_width, view_height = dim
	v_gap, h_gap = gap
	#horizontal (x) displacement of each view
...
def vert(view_list = list(), dim = (50,50), gap = (4,4)):
	view_width, view_height = dim
...
3, aspect_ratio = 1, gap = 4):
	...
	if parent_view is None:
		...
		import math
		i = views_per_row
		view_width = math.floor(parent_view.width/i) - (i + 1) * gap
		view_height = view_width * aspect_ratio
		
		x_gap = (parent_view.width - (view_width*i))/(i+1)
	...
	row = lambda i: i // views_per_row
	y = lambda i: row(i) * view_height + (row(i) + 1) * gap
